...
# Script en el cual se van a preparar los datos para introducirlos en la STNN en rango diarios. En primer lugar, se va
...
# Script en el cual se van a preparar los datos para introducirlos en la STNN en rango horario. En primer lugar, se va
# a realizar un dataset sencillo. Para ello:
# 1_ Cargar datos y paquetes
library(data.table); library(lubridate); library(sp); library(plyr)
load("../Accidentes de tráfico - Madrid/Cleaned_data/GeoAccidentalidad.RData")
...
car_crash <- unique(join(car_crash, zonas))
car_crash <- na.omit(car_crash)
car_crash <- car_crash[, .N, by = list(FECHA, `RANGO HORARIO`, zona)]
...
number_crash[is.na(number_crash)] <- 0
# Además se recoloca para poder hacer el reshape más adelante
# number_crash$rango <- seq(1, nrow(number_crash),1)
# number_crash <- number_crash[, c(5,3,4,1,2)]
# number_crash[ , c(4,5) := NULL]

# 10_ Ahora simplemente se pone en forma de matriz para la entrada en la STNN mediante dcast de data.table
crash <- dcast(number_crash, FECHA + `RANGO HORARIO` ~ zona)
